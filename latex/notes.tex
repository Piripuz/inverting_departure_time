...
\begin{equation}
  ...
  \mathbb{P}(t^* \in (b_i(\beta), t_s(\beta, \gamma)), T^* = t^*) = \prob
\end{equation}
...
This seems like it is working!! Let's extend it to the case in which \textcolor{orange}{\(\gamma\) and \(\beta\) interact}.

\begin{align*}
    f_{T_a, \hat{Q}}(t_a, -1) & = \mathbb{P}(t^* \in (b_i(\beta), t_s(\beta, \gamma)) | b_i(\beta) = t_a)f_\beta(tt_a'(t_a))[tt_a''(t_a)]^+ \\
  & = \mathbb{P}(t^* \in (t_a, t_s(tt_a'(t_a), \gamma)))f_\beta(tt_a'(t_a))[tt_a''(t_a)]^+ \\
  & = \int_1^\infty f_\gamma(x) (F_{T^*}(t_s(tt_a'(t_a), x)) - F_{T^*}(t_a)) dx\ f_\beta(tt_a'(t_a))[tt_a''(t_a)]^+
\end{align*}

Similarly, for late arrivals,

\begin{equation*}
  f_{T_a, \hat{Q}}(t_a, 1) = \int_0^1 f_\beta(x) (F_{T^*}(t_a) - F_{T^*}(t_s(x, -tt_a'(t_a))) ) dx\ f_\beta(tt_a'(t_a))[tt_a''(t_a)]^+
\end{equation*}
...
\begin{multline}
  ...
  f_{T_a}(t_a) = f_{T^*}(t_a)\int_{\beta_0(t_a)}^{\beta_\text{max}}f_\beta(b)\, db\int_{\gamma_0(t_a)}^{\gamma_\text{max}}f_\gamma(g)\, dg \\
  + \int_1^\infty f_\gamma(x) (F_{T^*}(t_s(tt_a'(t_a), x)) - F_{T^*}(t_a)) dx\ f_\beta(tt_a'(t_a))[tt_a''(t_a)]^+ \\
  + \int_0^1f_\beta(x) (F_{T^*}(t_a) - F_{T^*}(t_s(x, -tt_a'(t_a))) ) dx\ f_\beta(tt_a'(t_a))[tt_a''(t_a)]^+  
\end{multline}
